...
#'
#' Useful to split a data value into random parts that will sum back to the
#'   total.
#' @param b numeric; the number of bins to generate.
#' @param s numeric; the seed for randomly generating numbers.
...
#' @export
random_percents <- function(b = NULL, s = 1) {
  if (is.numeric(s)) {
    set.seed(s)
  } else {
